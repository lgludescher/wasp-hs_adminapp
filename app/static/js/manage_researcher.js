...
const ROLE_DISPLAY = { 2: 'PhD Student', 3: 'Postdoc' };
const ROLE_PATHS = { 2: 'phd-students', 3: 'postdocs' };

// --- DOM Elements ---
const detailsSection = document.getElementById('researcher-details-section');
...
(async function init() {
  ...
  researcherId = parseInt(pathParts[pathParts.length - 1], 10);

  if (!researcherId) {
    return showError("Could not determine a valid Researcher ID from the URL.");
  }
  ...
  try {
    ...
    showError(err);
    detailsSection.innerHTML = `<p>Error loading researcher details. Please try again.</p>`;
  }
...
async function loadAndRenderMainDetails() {
  try {
    researcherData = await apiFetch(`/researchers/${researcherId}/`);
    if (!researcherData || !researcherData.person_role) {
      throw new Error("Failed to load researcher data or researcher is missing a role.");
    }
    renderMainDetails();
  } catch (err) {
    showError(err);
...
function renderMainDetails(isEdit = false) {
  const { person_role: role } = researcherData;
  ...
  if (isEdit) {
    detailsSection.innerHTML = `
      <div class="details-box-content">
        <div class="detail-group">
            ...
            <label class="detail-item"><strong>Title:</strong> <select name="title_id"></select></label>
        </div>
        <div class="detail-group">
            <div class="detail-item"><strong>Email:</strong> <span>${person.email}</span></div>
            <label class="detail-item"><strong>Personal Link:</strong> <input name="link" type="url" value="${researcherData.link || ''}"></label>
        </div>
        <div class="detail-group">
        ...
        <div class="detail-group">
            <label class="detail-item"><strong>Original Title:</strong> <select name="original_title_id"></select></label>
            <label class="detail-item"><strong>Notes:</strong> <textarea name="notes">${researcherData.notes || ''}</textarea></label>
        </div>
      </div>
    ...
    populateDropdown(detailsSection.querySelector('[name="title_id"]'), '/researcher-titles/', 'title', researcherData.title?.id);
    populateDropdown(detailsSection.querySelector('[name="original_title_id"]'), '/researcher-titles/', 'title', researcherData.original_title?.id);
    ...
    detailsSection.innerHTML = `
      ...
      <div class="details-box-content">
        <div class="detail-group">
            <div class="detail-item"><strong>Name:</strong> <span>${person.first_name} ${person.last_name}</span></div>
            <div class="detail-item"><strong>Title:</strong> <span>${researcherData.title?.title || ''}</span></div>
        </div>
        <div class="detail-group">
            <div class="detail-item"><strong>Email:</strong> <span>${person.email}</span></div>
            <div class="detail-item"><strong>Personal Link:</strong> ${renderAsConditionalLink(researcherData.link)}</div>
        </div>
        <div class="detail-group">
        ...
        <div class="detail-group">
            <div class="detail-item"><strong>Original Title:</strong> <span>${researcherData.original_title?.title || ''}</span></div>
            <div class="detail-item"><strong>Notes:</strong> <span>${researcherData.notes || ''}</span></div>
        </div>
      </div>`;
...
async function saveMainDetails() {
  const titleId = parseInt(detailsSection.querySelector('[name="title_id"]').value, 10);
  const originalTitleId = parseInt(detailsSection.querySelector('[name="original_title_id"]').value, 10);

  const researcherUpdate = {
    title_id: titleId || null,
    original_title_id: originalTitleId || null,
    link: detailsSection.querySelector('[name="link"]').value.trim() || null,
    notes: detailsSection.querySelector('[name="notes"]').value.trim() || null
  };
  const roleUpdate = {
    start_date: detailsSection.querySelector('[name="start_date"]').value || null,
    end_date: detailsSection.querySelector('[name="end_date"]').value || null
  };
  try {
    await apiFetch(`/researchers/${researcherId}/`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(researcherUpdate) });
    await apiFetch(`/person-roles/${researcherData.person_role_id}/`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(roleUpdate) });
    await loadAndRenderMainDetails();
  } catch (err) {
    showError(err);
  }
...
async function loadProjects(panel) {
  ...
  try {
    const projectLinks = await apiFetch(`/person-roles/${researcherData.person_role_id}/projects/`);
...
function startEditProjectRow(tr, item) {
  ...
  tr.querySelector('.save-btn').onclick = async () => {
    const data = {
        person_role_id: researcherData.person_role_id,
    ...
    try {
      const updatedItem = await apiFetch(`/projects/${item.project.id}/people-roles/${researcherData.person_role_id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
...
async function setupProjectsAddForm(panel) {
    ...
    // Helper function to fetch, sort, and populate the final project list
    const updateProjectDropdown = async () => {
        ...
        // Sort the results by project_number on the frontend
        projects.sort((a, b) => {
    ...
    // Populate the Call Type filter and set up event listeners
    ...
    // Initial population of the project list based on default filters ("Ongoing")
    await updateProjectDropdown();

    // Set up the main form submission logic
    setupPanelAddForm(panel, {
      loadFn: loadProjects,
      addCallback: (formData) => apiFetch(`/projects/${formData.get('project_id')}/people-roles/`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ person_role_id: researcherData.person_role_id, is_principal_investigator: formData.get('is_pi') === 'on', is_contact_person: formData.get('is_contact_person') === 'on' })
      }),
    });
}
...
async function loadInstitutions(panel) {
  ...
  try {
    const institutionLinks = await apiFetch(`/person-roles/${researcherData.person_role_id}/institutions/`);
...
function startEditInstitutionRow(tr, item) {
    ...
    tr.querySelector('.save-btn').onclick = async () => {
        ...
        try {
            const updatedLink = await apiFetch(`/person-roles/${researcherData.person_role_id}/institutions/${item.institution.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
...
async function setupInstitutionsAddForm(panel) {
    ...
    setupPanelAddForm(panel, {
        ...
        addCallback: (formData) => apiFetch(`/person-roles/${researcherData.person_role_id}/institutions/`, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
async function loadFields(panel) {
  ...
  try {
    const fields = await apiFetch(`/person-roles/${researcherData.person_role_id}/fields/`);
...
async function setupFieldsAddForm(panel) {
    ...
    setupPanelAddForm(panel, {
        loadFn: loadFields,
        addCallback: (formData) => apiFetch(`/person-roles/${researcherData.person_role_id}/fields/`, {
            method: 'POST',
...
async function loadSupervisees(panel) {
  ...
  try {
    const supervisions = await apiFetch(`/person-roles/${researcherData.person_role_id}/students/`);
...
async function setupSuperviseesAddForm(panel) {
    ...
    const populateStudents = async () => {
        ...
        // Construct the URL to fetch ONLY active students using the backend's query parameter
        ...
        activeStudents.forEach(student => {
           ...
           // The value for the option should be the student's person_role_id
    ...
    setupPanelAddForm(panel, {
        ...
        addCallback: (formData) => apiFetch(`/person-roles/${researcherData.person_role_id}/students/`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                supervisor_role_id: researcherData.person_role_id,
...
async function loadCoursesTeaching(panel) {
  ...
  try {
    ...
    if (!courses.length) {
      ...
      tbody.innerHTML = `<tr><td colspan="5">This researcher is not associated with any courses.</td></tr>`;
    } else {
...
async function setupCoursesTeachingAddForm(panel) {
    ...
    const updateCourseList = async () => {
        const params = new URLSearchParams();
        // Default to only showing courses in active terms
    ...
    termFilter.onchange = () => {
        if (termFilter.value) activityFilter.value = ''; // Reset other filter
        updateCourseList();
    };
    activityFilter.onchange = () => {
        if (activityFilter.value) termFilter.value = ''; // Reset other filter
        updateCourseList();
    };

    await updateCourseList(); // Initial population
...
function startEditDecisionLetterRow(tr, item) {
    ...
    tr.querySelector('.save-btn').onclick = async () => {
        ...
        try {
            const updatedItem = await apiFetch(`/person-roles/${researcherData.person_role_id}/decision-letters/${item.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(data)
            });
...
function setupDecisionLettersAddForm(panel) {
    setupPanelAddForm(panel, {
        ...
        addCallback: (formData) => apiFetch(`/person-roles/${researcherData.person_role_id}/decision-letters/`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
function showModal(message) { modalText.textContent = message; modalConfirmBtn.style.display = ''; modalCancelBtn.textContent = 'Cancel'; modal.classList.remove('hidden'); }
async function confirmRemove() {
  if (!pendingRemove) return;
  const { type, item } = pendingRemove;
  try {
    if (type === 'project') {
      await apiFetch(`/projects/${item.project.id}/people-roles/${researcherData.person_role_id}`, { method: 'DELETE' });
      await loadProjects(document.getElementById('panel-content-projects'));
    } else if (type === 'institution') {
      await apiFetch(`/person-roles/${researcherData.person_role_id}/institutions/${item.institution.id}`, { method: 'DELETE' });
      await loadInstitutions(document.getElementById('panel-content-institutions'));
    } else if (type === 'field') {
      await apiFetch(`/person-roles/${researcherData.person_role_id}/fields/${item.id}`, { method: 'DELETE' });
      ...
      await apiFetch(`/courses/${item.id}/teachers/${researcherData.person_role_id}`, { method: 'DELETE' });
      await loadCoursesTeaching(document.getElementById('panel-content-courses'));
    } else if (type === 'decision_letter') { // Add this 'else if' block
      await apiFetch(`/person-roles/${researcherData.person_role_id}/decision-letters/${item.id}`, { method: 'DELETE' });
      await loadDecisionLetters(document.getElementById('panel-content-letters'));
    }
    pendingRemove = null; modal.classList.add('hidden');
  } catch (err) { showError(err); }
}
function showError(err) { modalText.textContent = err.message || String(err); modalConfirmBtn.style.display = 'none'; modalCancelBtn.textContent = 'OK'; modal.classList.remove('hidden'); }
...
/**
 * Renders a string as a clickable link only if it's a valid absolute URL.
 * Otherwise, renders it as plain text.
 * @param {string | null} link The string to render.
