...
def list_academic_publications(
    ...
    logger.info(f"{current_user.username} listed academic publications (year={published_year}, "
                f"is_relevant={is_relevant}, is_reviewed={is_reviewed}, search={search!r})")
...
@router.post("/academic-publications/actions/sync-swepub")
def sync_swepub(
    db: Session = Depends(dependencies.get_db),
    current_user=Depends(dependencies.get_current_user)
):
    """Placeholder: Trigger API fetch from SwePub based on active researchers."""
    logger.info(f"{current_user.username} triggered SwePub sync")
    return {"status": "Not implemented", "message": "Will fetch latest records from SwePub"}


@router.post("/academic-publications/actions/process-pending")
def process_pending_academic(
    db: Session = Depends(dependencies.get_db),
    current_user=Depends(dependencies.get_current_user)
):
    """Placeholder: Trigger evaluation or formatting for unreviewed academic items."""
    logger.info(f"{current_user.username} triggered processing for pending academic publications")
    return {"status": "Not implemented", "message": "Will process unreviewed publications"}
